...
static void TheKingPrintBytesGivenStartEndPointer(char* pMessage, uint8_t* pStart, uint8_t* pEnd, int iMaxBytes = -1)
{
    printf("TheKing--> %s", pMessage);
...
static bool check_bits[20000*8];
static void TheKingPrintBitsGivenStartEndBytePointer(char* pMessage, uint8_t* pStart, uint8_t* pEnd, int iMaxBytes = -1)
{
    int check_bits_idx = 0;
    uint8_t *pSrcRbsp = pStart;
    int iDx = 0;
    while(pSrcRbsp!=pEnd)
    {
        for(int j=7;j>=0;j--)
        {
            if( *pSrcRbsp & (1<<j))
                check_bits[check_bits_idx++] = 1;
            else
                check_bits[check_bits_idx++] = 0;
        }
    ...
    printf("TheKing--> %s, (TotalBits = %d): ", pMessage, check_bits_idx);
    for(int i=0;i<check_bits_idx;i++)
    {
        printf("%d", check_bits[i]);
        if((i+1)%4==0) printf(" ");
        if((i+1)%8==0) printf(" ");
    }
    printf("\n");
...
{
    //printf("TheKing inside demux_ts_packet\n");
    uint8_t *pBuff = (uint8_t*)ptr;
    
    TheKingPrintBitsGivenStartEndBytePointer("Packet: ", pBuff, pBuff+188,  4);
    ...
    if(adaptation_field_exist)
    {
        ...
        if(ptr>=end_ptr)
        {
            printf("Pointer reached enad point\n");
            return -3;
        }
    }
#ifdef VERBOSE
...
void ts::demuxer::show(void)
{
    ...
    for(std::map<u_int16_t,ts::stream>::const_iterator i=streams.begin();i!=streams.end();++i)
    {
        ...
        if(s.type!=0xff)
        {
            ...
            if(s.frame_length>0)
                printf(", fps=%.2f",90000./(double)s.frame_length);
            
            if(len>0)
                printf(", len=%llums",len/90);
            
            
            if(s.frame_num>0)
                printf(", fn=%llu",s.frame_num);
            
            u_int64_t esfn=s.get_es_frame_num();
            
            if(esfn>0)
                printf(", esfn=%llu",esfn);
...
//    prefix.clear();
    ...
    printf("TheKing--> Inside demux_file\n");
